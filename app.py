...
2. **[누락 절대 금지]** 음성 기록에 한 번이라도 언급된 통증 부위(허리, 목, 어깨, 무릎 등)나 증상은 **절대로 생략하거나 누락하지 마세요.** 반드시 각 부위별로 `#Low back pain`, `#Posterior neck pain` 등 독립된 진단 섹션을 모두 생성해야 합니다. AI가 임의로 덜 중요하다고 판단하여 특정 부위(예: 허리)의 진단을 통째로 빼먹는 일이 없도록 하세요.
...
5. '부위' 섹션은 '좌', '우', '좌/우', '요천추' 등 핵심 키워드만 사용하여 최대한 간결하게 작성하세요.
   - 특히 허리 통증의 경우, 좌측이나 우측의 통증이 '동반증상'의 방사통으로 충분히 표현된다면 해당 진단 섹션의 '부위' 항목에는 **'요천추'** 등 주된 통증 부위만 간결히 기입하세요. (진단 자체를 생략하라는 의미가 아닙니다)
   - **양측 모두 아픈 경우 '중앙/양측' 대신 '좌/우'를 사용하세요.**
6. '양상' 섹션은 '묵직함', '쑤심', '뻐근함', '알싸함' 등의 한의학 용어를 우선적으로 사용하세요.
   - **주의:** '열감'이나 '부종'이 있는 경우 '양상' 섹션에 적지 말고, 반드시 '동반증상' 섹션 끝에 '열감(+) / 부종(+)' 형태로 기입하세요.
...
8. '동반증상' 섹션은 각 부위별 체크 템플릿을 사용하되, (좌/우)나 (둔부/하지)와 같이 구분된 항목은 음성에서 확인된 결과에 따라 (+/+), (+/-), (-/+), (-/-)와 같이 각각의 유무를 명확히 표시하세요.
   - 예: 방사통(좌/우)(-/-) / 저림(좌/우)(-/-) / 감각저하(-) / 근력저하(-) / 열감(+) / 부종(+)
9. o/s(사고 상황) 섹션 작성 시 아래 형식을 엄격히 따르세요.
   - **교통사고(TA)의 경우:**
     * 형식: o/s - [날짜]. [장소]에서 [상황]한 TA([충돌유형], [본인위치])(환자진술에 근거함)
     * 예: 시청사거리 상무지구 방향 교차로에서 주행 중 옆 차선에서 끼어들면서 추돌한 TA(car+car, 운전석)(환자진술에 근거함)
     * 주의: '추돌하여 수상(조수석 탑승)'과 같이 작성하지 말고, 반드시 '추돌한 TA(car+car, 조수석 탑승)'과 같이 'TA' 단어와 충돌유형을 포함하세요.
   - **일반 사고(자전거, 낙상 등)의 경우:** '넘어진 사고' 등으로 명사형으로 끝내지 말고, 반드시 '~하여 수상함'으로 문장을 끝맺으세요.
     * 예: o/s - 2025.11.26. 자전거를 타고 가다가 넘어져서 수상함 (2026.03.16. 후방십자인대 파열 수술 시행)
10. **[핵심] 임상 용어 및 ROM 작성 (필수 준수)**:
    - **[중요] 진단한 모든 통증 부위(목, 허리, 어깨 등)에 대하여, 음성에서 가동범위(ROM)에 대한 언급이 아예 없거나 제한이 없다고 확인된 경우, 차트 하단에 무조건 'ROM 제한없음.' 이라고 기본으로 출력하세요.** (예: 목 -> C-spine: ROM 제한없음. / 허리 -> L-spine: ROM 제한없음. / 어깨 -> Shoulder: ROM 제한없음.)
    - 단, 음성에서 특정 동작(굴곡, 신전 등)의 제한이 명시적으로 언급된 경우에는 해당 제한 사항만 기입하고, 언급되지 않은 동작 제한을 임의로 추가하지 마세요.
